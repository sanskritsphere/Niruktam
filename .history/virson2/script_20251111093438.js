...
// सिर्फ left-labels के लिए b...b unwrap का regex (बाकी tags नीचे अलग तरीके से संभलते हैं)
...
/* ========= collect all tags (b/m/v/...) into anchors + termsByTag ========= */
...
function populateSidebar() {
  ...
  data.forEach((sutra, index) => {
    ...
    const title = base;
    const label = base; // चाहो तो slice कर सकते हैं

    const li = document.createElement('li');
    li.textContent = label;          // plain
    li.title = title;
    li.dataset.baseLabel = label;    // restore के लिए stash
  ...
  fetch("data.json")
    ...
    .then(json => {
      data = json.niruktam.map((item, i) => {
        ...
        // Main HTML with anchors for ALL configured tags
