...
function dragMove(e) {
  if (isDragging) {
    ...
    // Boundary check (स्क्रीन से बाहर न जाए)
    const rect = panel.getBoundingClientRect();
    const maxX = window.innerWidth - rect.width - 20;
    const maxY = window.innerHeight - rect.height - 20;
    currentX = Math.max(-window.innerWidth + 50, Math.min(maxX, currentX));
    currentY = Math.max(-window.innerHeight + 50, Math.min(maxY, currentY));
